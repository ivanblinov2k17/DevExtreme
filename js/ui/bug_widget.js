...
class BugWidget extends Widget {
    constructor(...args) {
        super(...args);
    ...
    _initTemplates() {
        this._templateManager.addDefaultTemplates({
            content: new FunctionTemplate(({ model = {}, container }) => {
                ...
                const $textContainer =
                    text && $('<span>').text(text).addClass('dx-button-text');
        ...
        if(useInkRipple) {
            ...
            const changeWaveVisibility = (event, visible) => {
                ...
                if(useInkRipple && activeStateEnabled && !this._disposed) {
                    ...
                    visible
                        ? _inkRipple.showWave(config)
                        : _inkRipple.hideWave(config);
                }
            };
    ...
    _renderSubmitInput() {
        ...
        if(useSubmitBehavior) {
            ...
            clickEvent.on(this._$submitInput(), (event) =>
                submitAction({ event })
            );
        }
    }
    ...
    _updateClick() {
        this._clickAction = this._createActionByOption('onClick', {
            ...
            afterExecute: () => {
                ...
                useSubmitBehavior &&
                    setTimeout(() => this._submitInput().click());
            },
        });
    }
    ...
    _updateContent() {
        ...
        const $template = $(
            this._getTemplateByOption('template').render({
                ...
                transclude:
                    this._templateManager.anonymousTemplateName === template,
            })
        );
    ...
    _updateType() {
        ...
        ['back', 'danger', 'default', 'normal', 'success']
            .map((type) => `dx-button-${type}`)
            .forEach($element.removeClass.bind($element));
...
registerComponent('dxBugWidget', BugWidget);
